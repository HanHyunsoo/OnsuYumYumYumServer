...
public class RestaurantCategoryController {
    ...
    @Operation(
            ...
            description = "여러개의 카테고리를 request로 받아서 카테고리를 찾고 없으면 생성을 한다음 그 카테고리들을 해당 ID 음식점의 연관관계를 설정하고 저장합니다\n" +
    ...
    public ResponseEntity<SuccessResponseBody<Map<String, Object>>> saveAllRestaurantCategoryById(@PathVariable @Parameter(description = "음식점 ID") Long id, @RequestBody Set<String> categoryRequest) {
        if (categoryRequest.isEmpty()) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "카테고리를 한개 이상 요청 보내야 합니다.");
        }

        Map<String, Object> responses = restaurantCategoryService.saveAllRestaurantCategoryWithRequest(id, categoryRequest, true);
    ...
    public ResponseEntity<SuccessResponseBody<Page<CategoryResponse>>> findAllCategoryByRestaurantId(@PathVariable @Parameter(description = "음식점 ID") Long id, Pageable pageable) {
    ...
    public ResponseEntity<SuccessResponseBody<Page<RestaurantResponse>>> findAllRestaurantByCategoryId(@PathVariable @Parameter(description = "카테고리 ID") Long id, Pageable pageable) {
